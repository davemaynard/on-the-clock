...
export function nextPick(picks, drafted) {
...
export function survives(better, picks) {
  if (picks <= 0) return 1;
...
export function marketQueue(players, drafted) {
...
/** Open starting slots by position, and how many flex-family slots are still open. */
export function openSlots(league, players, mine) {
  const owned = {};
  for (const index of mine) {
    const pos = players[index].pos;
    owned[pos] = (owned[pos] || 0) + 1;
  }
  const open = {};
  for (const pos of ALL_POSITIONS) {
    open[pos] = Math.max(0, (league.slots[pos] || 0) - (owned[pos] || 0));
  }
  ...
  for (const pos of SKILL_POSITIONS) {
    surplus[pos] = Math.max(0, (owned[pos] || 0) - (league.slots[pos] || 0));
  }
  const familyOpen = families.map((family) => {
    let remaining = family.count;
    for (const pos of family.eligible) {
      if (!remaining) break;
      const used = Math.min(remaining, surplus[pos] || 0);
      surplus[pos] -= used;
      remaining -= used;
    }
    return remaining;
 ...
 */
export function waitCosts({ players, drafted, queue, picksUntilFollowing, families }) {
 ...
 */
export function assess(league, players, state) {
  ...
  const inAnyFamily = (pos) => families.some((family) => family.eligible.includes(pos));
  ...
  const streamersNeeded = (open.K || 0) + (open.DST || 0);
  ...
  const fit = (i) => players[i].vor + (players[i].out ? 0 : bonus(players[i].pos));
  ...
  return {
    ...
    chance: (i) => survives(queue.get(i) ?? 0, picksAway),
  };
}
