...
export const verdictKind = (verdict) =>
  verdict.startsWith("AVOID") || verdict.startsWith("DO NOT") ? "avoid" : "stash";

/**
 * The tags one player shows, in order: `[{kind, text, title?}]`. Status and verdict
 * together ("out" + "do not draft") say one thing twice and cost the name its room; the
 * verdict tells you what to DO, so when both apply it stands alone.
 */
