 ...
 */
export function Board({ players, state, draft, score, fitMode, onToggleFit, dispatch }) {
  ...
  const onRowClick = (event, index) => {
    if (event.target.closest("[data-score]")) {
      onToggleFit();
      return;
    }
    dispatch({ type: event.target.closest("[data-claim]") ? "claim" : "tap", index });
    afterMark();
  };
  ...
  return (
    <>
      <Tools
        ...
        pos={pos}
        onPos={setPos}
        isMuted={(p) => draft.endgame && SKILL_POSITIONS.includes(p) && draft.exhausted(p)}
