...
from on_the_clock import board
...
# ESPN's lineup slots are numeric ids; the tests read better by name.
SLOT_IDS = {pos: str(sid) for sid, pos in board.SLOT_POS.items()}
SLOT_IDS.update({"FLEX": "23", "OP": "7", "BENCH": "20", "IR": "21"})


def lineup(size, **slots):
    """League settings for a `size`-team room with the named starting slots."""
    counts = {SLOT_IDS[name]: count for name, count in slots.items()}
    return {"size": size, "rosterSettings": {"lineupSlotCounts": counts}}
...
def test_standard_two_flex_league():
    ...
    s = lineup(10, QB=1, RB=2, WR=2, TE=1, FLEX=2, DST=1, K=1, BENCH=6)
    levels, detail = board.replacement_levels(pool(), s)
...
def test_superflex_league_puts_qbs_in_flex():
    ...
    s = lineup(12, QB=1, RB=2, WR=2, TE=1, OP=1, FLEX=1, DST=1, K=1, BENCH=7, IR=1)
    levels, detail = board.replacement_levels(pool(), s)
    taken = detail["starters_taken"]
...
def test_unknown_slot_id_refuses_instead_of_dropping_starters():
    ...
    try:
        board.replacement_levels(pool(), s)
...
def test_superflex_detection():
    ...
    assert board.is_superflex(sf)
    assert not board.is_superflex(std)
    assert board.adp_format(sf) == "2qb"
    assert board.adp_format(std) == "ppr"
